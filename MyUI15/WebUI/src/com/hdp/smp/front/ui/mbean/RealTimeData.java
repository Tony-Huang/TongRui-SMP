...
//        List<Station> allStations = sdao.getAll(session, com.hdp.smp.model.Station.class);
//        session.close();
...
//    }
    ...
    public void refreshData(PollEvent pollEvent) {
        //realtime data should not save in db. So should not read from DB, from monitor instead.
        Integer stNO = this.stationNO;
        if( stNO == null ||stNO <1 ) {
        log.warn("user select station :"+this.stationNO );
        return;
        }
        
       // Monitor mon = DataCache.getMonitor2(stationId);
        Monitor mon = DataCache.getMonitorByStNO(this.stationNO);
       log.info("Selected monitor="+mon);
       if (mon == null) {
           log.error(" no monitor found!!  ...");
           return;
           }
       
       //read data from monitor and set mbean's value
       RTDataService rtds = new RTDataService();
       StationData sd = rtds.readDataByStNO(this.stationNO);
       this.setRTData(sd);
...
//           }
    ...
    public void startFetchRTdata(ActionEvent actionEvent) {
       log.info("user want to read realtime data, input stationNO:"+this.stationNO);
    }
...
//    }
  ...
  //  private short[][] spindleSpeeds;

//    public void setSpindleSpeeds(short[][] spindleSpeeds) {
...
//    }
    ...
    public void readSpindleSpeed(ActionEvent actionEvent) {
        if (this.stationNO <1 ) return ;
        log.info("read spindle :"+this.getSpindleNO() +" speed " +" from station:"+this.stationNO  );
        //read data from monitor and set mbean's value
//        int spindleStartNO= this.spindleNORange(spindleNO)[0];
//        DataProvider_SpindleSpeed dp = DataProvider_SpindleSpeed.get();
//        dp.setStationNO(this.stationNO);
//        dp.setSpindlestartNO(spindleStartNO);
      //   short[][] matrix =   dp.getSpindleSpeedMatrix();
     //   this.setSpindleSpeeds(matrix);
    }
    ...
    /**
     *获取需要的最小和最大spindle NO. 比如spindle 21,相应spindle NO 范围为[1-50]
     * @param spindleNO
     * @return
     */
